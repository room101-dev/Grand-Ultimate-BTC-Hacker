...
/* msb of Y ETHEREUM coordinate of public key */


static void yhash160(hash160_t *h, const unsigned char *upub) {
  memcpy(h->uc, upub+1+32, 20); // lets really grab the y component
}
...
inline static void fprintresult2(FILE *f, hash160_t *hash,
                                unsigned char compressed,
                                unsigned char *type,
                                unsigned char *input,
                                unsigned char *priv ) {
  unsigned char hexed0[41];
  unsigned char hexed1[65];
  ...
  fprintf(f, "%s:%c:%s:%s:%s\n",
          ...
          hex(priv, 32, hexed1, sizeof(hexed1)),  
...
int main(int argc, char **argv) {
  ...
  int Popt = 0; // poc 03feb18
  uint64_t Hopt = 0; // poc 13feb18
  ...
  while ((c = getopt(argc, argv, "avxb:hi:k:f:m:n:o:p:s:r:c:t:w:I:B:P:H:")) != -1) {
    switch (c) {
      case 'a':
...
// main code loop begins here
  uint64_t loopcount=0; // keep track of total main loops

  for (;;) {
    loopcount++;
    // in Inopt mode allow a '-Halt' argument so that it doesn't loop forever
    if ( Iopt && Hopt && loopcount > Hopt ) break;
...
// poc 03feb18 added support for -P option to generate private-key hex
              //printf("Popt=%d", Popt);
              if ( Popt ) 
                fprintresult2(ofile, &hash160, pubhashfn[j].id, modestr, batch_line[i],batch_priv[i] );
              else
