...
// poc 27nov20 added debug externals
...
// poc 30nov20 place dump here to find new hash
...
// poc 30nov20 add another checkpoint here to see value of h1-h4
...
extern int bloom_chk( uint32_t * );

// poc 28nov20 added call routine too keep code outside of checkAddr

void dumpHash(Secp256K1 *secp, uint8_t *hash160, Int &key, bool mode) {
	printf("\n904@Dump\n");
	//dumphex(hash160,20);
	printf("Ripe: %s\n", ripemd160_hex(hash160).c_str() );
	
	//string paddr = secp->GetPrivAddress(mode ,key).c_str();
	//dumphex(pa,32);
	Point p = secp->ComputePublicKey(&key);
	printf("Priv: %s\n", secp->GetPrivAddress(mode ,key).c_str() );
	printf("Pub: %s\n",secp->GetPublicKeyHex(mode,p).c_str());
	printf("Addr: %s\n", secp->GetAddress(0, mode, hash160).c_str() );
	
	//GetHash160(int type, bool compressed, Point &pubKey, unsigned char *hash)
	char  hash[21];
	secp->GetHash160( P2PKH,0, /*secp->GetPublicKeyHex(mode,p)*/(Point &) p, (unsigned char*)&hash );
	printf("Pub2Hash(u): " ); dumphex( (const unsigned char *) &hash[0], 20);
 	secp->GetHash160( P2PKH,1, /*secp->GetPublicKeyHex(mode,p)*/(Point &) p, (unsigned char*)&hash );
	printf("Pub2Hash(c): " ); dumphex( (const unsigned char *) &hash[0], 20);

   //printf("Cpu Bloom(%d): ",bloom_chk((uint32_t *)&hash));

    fflush(stdout);

	fprintf( fpbout, "\n%s:",secp->GetPrivAddress(mode ,key).c_str() );
	fdumphex( fpbout, (const unsigned char *) &hash[0], 20);
    fflush(fpbout);
...
// poc dump 27nov20
    //dumpHash( this->secp, hash160, key, mode ); 
	//dumphex(hash160,20); debug

	Point p = secp->ComputePublicKey(&key);
    char  hash[21];
    // un-compressed
    secp->GetHash160(P2PKH,0,(Point &)p, (unsigned char*)hash ); // routine void passes result to hash

    if (  bloom_chk( (uint32_t *)hash ) ) {
        //dumphex(hash,20);
        dumpHash( this->secp, hash, key, mode ); // pass required arg's
        nbFoundKey++;

    }
    // compressed
    secp->GetHash160(P2PKH,1,(Point &)p, (unsigned char*)hash ); // routine void passes result to hash
    ...
    // ripe160
    if (  bloom_chk( (uint32_t *) hash160 ) ){
        //dumphex(hash,20);
        dumpHash( this->secp, hash160, key, mode ); // pass required arg's
        nbFoundKey++;
    }

    return; // poc test, no effect for the record same speed
