...
// poc 27nov20 add file support for keeping found hash 
...
const unsigned char *thex="1b35f12fcfb9d6e60c5383574127d77a743e37ce"; // high value hash

extern void bloom_alloc(char *);
extern uint64_t test_bloom(void);
extern void gethex( unsigned char * , int len, unsigned char *) ;
...
void setup_bloom()
{
	...
	fpbout = fopen( bout, "a+" );
     ...
     bloom_alloc("/home/master/Crypto/hex2blf/monster8.blf");
     // test bloom
    int ret=test_bloom();
    printf("bloom test=%lld\n",ret);

    unsigned char thash[20];
    unsigned char *tp = &thash[0];
    printf("BLOOM-TEST[prefix_init]: %s: \n",thex); 
    gethex(thex,20,tp ); // poc disabled 10nov20
    //hash160_t *hp=thash; // poc '' was hp=&thash[0]
    printf("Cpu Bloom Check(%d): ",bloom_chk((uint32_t *)tp));
    //printf("Cpu Bloom Check(%d): ",bloom_chk((uint32_t *)&tp)); // the & case fails
}
