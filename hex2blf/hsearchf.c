...
//posix_fadvise(fileno(f), ((X*HASHLEN)&0xfffff000ULL)-4096ULL, 8192, POSIX_FADV_WILLNEED); \

#define READ_AT(X, H) do { \
  posix_fadvise(fileno(f), (X*HASHLEN), 8192, POSIX_FADV_WILLNEED); \
...
int hsearchf(FILE *f, hash160_t *hash) {
  int64_t ret, res = 0, i = 0; // was size_t
  int64_t file_sz; // was size_t
  struct stat sb;
