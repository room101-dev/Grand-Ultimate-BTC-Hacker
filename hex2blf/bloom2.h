...
//poc default int to long ( 1 to 1ULL ), when > 1GB, int32->int64
...
//#define BLOOM_SET_BIT(N) (bloom[(N)>>2] = bloom[(N)>>2] | (0x1UL<<((N)&0x7UL)))
...
//#define BLOOM_GET_BIT(N) ( ( bloom[(N)>>2]>>((N)&0x7UL) )&0x1UL)
