...
unsigned int chk_bloom(unsigned char* bloom)
{
	  uint64_t i;

  for (  i=0;i<BLOOM_SIZE;i++) 
	if (bloom[i] == 0x1 ) return(i);
  return(0xffffffff);
}
...
//printf("bh00-t bloom[(N)>>1] | (0x1ULL<<((N)&0x7UL))) %lx %lx %ld\n",t,(t)>>1, (0x1ULL<<((t)&0x7UL)) );
...
//printf("bh19-t bloom[(N)>>1] | (0x1ULL<<((N)&0x7ULL))) %ld %ld %ld\n",t,(t)>>1, (0x1ULL<<((t)&0x7ULL)));
