...
//poc increased m_bits 4x, k_hashes +10
const double k_hashes = 20;

//const double m_bits   = 2*4294967296;
const double m_bits   = 8ULL*4294967296;
...
//poc debug
//printf("RIPEMD160_DIGEST_LENGTH=%d\n",RIPEMD160_DIGEST_LENGTH>>2);
  ...
  if (stat(bloomfile, &sb) == 0) {
    if (!S_ISREG(sb.st_mode) || sb.st_size != BLOOM_SIZE) {
      fprintf(stderr, "[!] Bloom filter file '%s' is not the correct size (%ju != %lld)\n", bloomfile, sb.st_size, BLOOM_SIZE);
      exit(1);
    }
    ...
    printf("chkbloom(%llu)-chkbloom2(%llu)\n",chk_bloom(bloom),chk_bloom2(bloom) );
