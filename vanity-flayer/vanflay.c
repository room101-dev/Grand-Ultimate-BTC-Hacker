...
// poc add support for bloom-file
...
int Bopt=0;
FILE * fbopt=NULL;
int bfound=0; // signal that hash160 match found

unsigned char FOUND_PRIV[32];
unsigned char FOUND_HEX[32];


// poc add hash160 type for bloom
#include "hash160.h"
...
// poc keep last private key seen
...
//poc  debug added to dump hex
/*
	printf("\nPRIV (rekey=%d) ", rekey_at );
	printf("\nPRIV "); dumpbn(EC_KEY_get0_private_key(pkey));
	printf("\nbntmp "); dumpbn(&vxcp->vxc_bntmp);
	printf("\nbntmp2 "); dumpbn(&vxcp->vxc_bntmp);
*/
			EC_POINT_copy(ppnt[0], EC_KEY_get0_public_key(pkey));
			vg_exec_context_downgrade_lock(vxcp);
  //  printf("\nPRIV2 "); dumpbn(EC_KEY_get0_private_key(pkey));
	...
	// poc outside of Bopt testing

	hash160_t hash160;
	memcpy(&hash160,&vxcp->vxc_binres[1],20);

	if ( Bopt ) {

		///printf("\nH160 ");dumphex(&hash160,20 ); fflush(stdout);
...
/*
    EC_KEY *ec_key=pkey;
	EC_KEY_set_public_key(ec_key, ppnt[i]);
	EC_KEY_generate_key(ec_key);

	len = i2o_ECPublicKey(ec_key, &pend);
	//printf("\nPUB[%d] ",len);
	//dumphex(key_buf,len);
	SHA256(key_buf, len, hash1);
	RIPEMD160(hash1, sizeof(hash1), &hash160);
*/	
	//len = i2d_ECPrivateKey(ppnt[i], &pend);
	//printf("\nPriv "); //dumphex(pkey,32);
	/*
		BN_clear(&vxcp->vxc_bntmp);
		BN_set_word(&vxcp->vxc_bntmp, vxcp->vxc_delta);
		BN_add(&vxcp->vxc_bntmp2,
		EC_KEY_get0_private_key(vxcp->vxc_key),&vxcp->vxc_bntmp);
		printf("\nPriv-b "); dumphex(&vxcp->vxc_bntmp2,32);
	*/
	
	//printf("\nPriv-c "); dumpbn(EC_KEY_get0_private_key(vxcp->vxc_key));

	//printf("\nH160b ");dumphex(&hash160.uc,20 ); fflush(stdout);

//	memcpy(&hash160,&vxcp->vxc_binres[1],20);
//	printf("\nH160a ");dumphex(&vxcp->vxc_binres[1],20 ); 

/*
		if (bloom_chk_hash160(bloom, hash160.ul)) {

				fdumphex(stderr,&hash160.uc,20 ); fflush(stderr);
	//			if ( fbopt ) {
					fdumphex(fbopt,&hash160.uc,20 );
					fprintf(fbopt,":");
					fdumphex(fbopt,key_buf, len);
					fprintf(fbopt,":");
					fdumpbn(fbopt,EC_KEY_get0_private_key(vxcp->vxc_key));
...
//			memcpy(&hash160,&vxcp->vxc_binres[1],20);
			if (bloom_chk_hash160(bloom, hash160.ul)) {
				// very important here to lock, and save current state
				vg_exec_context_upgrade_lock(vxcp);
				bfound = 2;
				memcpy(FOUND_HEX,&hash160,20);
				//printf("\nH160:[vanflay] ");		
				fdumphex(stdout,FOUND_HEX,20 ); fflush(stdout);
				int tf= test_func(vxcp);
				//printf("**BFOUND tf(%d): ",tf ); // returns 2
				//fflush(stdout);
				//printf("Priv: ");
				//dumpbn(EC_KEY_get0_private_key(vxcp->vxc_key));
				
		
// seems to work better with 'goto out' disabled this way it keeps running forever				
		//		goto out; // dont' run test_func() again
...
main(int argc, char **argv)
{
	...
	int i;
// poc add -b bloom switch
	int ret;
	unsigned char *bopt = "vg.blf";
	while ((opt = getopt(argc, argv, "bvqnrik1eE:P:C:X:Y:F:t:h?f:o:s:")) != -1) {
		switch (opt) {
    ...
    // make sure its a .txt file as it contains hex addr & hex priv-key
	fbopt = fopen("found-vg.txt","a");
				break;
		case 'c':
		        compressed = 1;
		        break;
